...
function AdminTenderDetails() {
    ...
    const handleSelect = async (eventKey) => {
        ...
        const response = await axios.post('https://sgqfr6s6f8.execute-api.ap-south-1.amazonaws.com/production/api/tender/user/post', {
            usertender: eventKey
        }, {
            headers: {
    ...
    //     const response = await axios.get('https://sgqfr6s6f8.execute-api.ap-south-1.amazonaws.com/production/api/gettender');
    //     console.log(response)
    ...
    useEffect(() => {
        fetch('https://sgqfr6s6f8.execute-api.ap-south-1.amazonaws.com/production/api/gettender')
    ...
    const handleSelect1 = (tenderId) => {
        ...
        axios.post('https://sgqfr6s6f8.execute-api.ap-south-1.amazonaws.com/production/api/tender/user/find', { name: tenderId }, {
    ...
    const handleAllotmentChange = async (event) => {
        ...
        try {
            const data = await axios.put(`https://sgqfr6s6f8.execute-api.ap-south-1.amazonaws.com/production/api/update/${userToUpdate._id}`, { alloted: isAlloted });
            if (data.status === 200) {
        ...
        //     try {
        //         const data = await axios.put(`https://sgqfr6s6f8.execute-api.ap-south-1.amazonaws.com/production/api/update/${userToUpdate._id}`, { alloted: event.target.value });
        //         console.log(data);
        //     } catch (error) {
        //         console.error('Error:', error);
        //     }
    ...
    const handlePaidStatusChange = async (event) => {
        ...
        try {
            const data = await axios.put(`https://sgqfr6s6f8.execute-api.ap-south-1.amazonaws.com/production/api/update/${userToUpdate._id}`, { paid: isPaid });
                                        ...
                                        <TableBody>
                                            {tenderDetails.map((detail) => (
                                                <TableRow key={detail._id}>
                                                    <TableCell>{detail.name.name}</TableCell>
                                                    <TableCell>{detail.name.title}</TableCell>
                                                    <TableCell>{detail.name.State}</TableCell>
                                                    <TableCell>{detail.name.tender_value}</TableCell>
                                                    <TableCell>{detail.name.property_history}</TableCell>
                                                    <TableCell>{detail.name.scheme}</TableCell>
                                                    <TableCell>{detail.name.Emd}</TableCell>
                                                    <TableCell>{detail.name.description}</TableCell>
                                                    <TableCell>{detail.name.gross_area}</TableCell>
                                                    <TableCell>{detail.name.ward}</TableCell>
                                                    <TableCell>{detail.name.cts_number}</TableCell>
                                                    <TableCell>{new Date(detail.name.startDate).toLocaleDateString()}</TableCell>
                                                    <TableCell>{new Date(detail.name.endDate).toLocaleDateString()}</TableCell>
                                                    <TableCell>{new Date(detail.name.createdAt).toLocaleDateString()}</TableCell>
                                                    <TableCell>
                                                        {detail.name.doc && (
                                                            <Button variant="contained" color="primary" href={detail.name.doc}>
                                                                View Document
                                                            </Button>
                                                        )}
                                                    </TableCell>
