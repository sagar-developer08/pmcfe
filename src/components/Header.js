...
function Header() {
  ...
  const me = async () => {
    ...
    if (token) {
      const response = await axios.get('https://sgqfr6s6f8.execute-api.ap-south-1.amazonaws.com/production/api/me', {
        headers: {
          Authorization: `${token}`
        }
      });
  ...
  const handleSave = async () => {
    const token = localStorage.getItem('token');
    if (token) {
      const response = await axios.put(`https://sgqfr6s6f8.execute-api.ap-south-1.amazonaws.com/production/api/admin/user/update/${user._id}`, {
        phone: phoneNumber
      }, {
        headers: {
          Authorization: `${token}`
